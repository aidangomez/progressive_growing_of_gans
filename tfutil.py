...
def is_tf_expression(x):
  return isinstance(x, tf.Tensor) or isinstance(x, tf.Variable) or isinstance(
      x, tf.Operation)
...
def finalize_autosummaries():
  ...
  init_uninited_vars(
      [var for vars in _autosummary_vars.values() for var in vars])
...
def _create_autosummary_var(name, value_expr):
  ...
  update_op = tf.cond(
      tf.is_variable_initialized(var), lambda: tf.assign_add(var, v),
  ...
  def register_gradients(self, loss, vars):
    ...
    with tf.name_scope(self.id + '_grad'), tf.device(dev):
      ...
      grads = self._dev_opt[dev].compute_gradients(
          loss, vars, gate_gradients=tf.train.Optimizer.
          GATE_NONE)  # disable gating to reduce memory usage
      grads = [
        ...
        with tf.name_scope('ApplyGrads%d' % dev_idx), tf.device(dev):
          ...
          with tf.name_scope('UpdateWeights'):
            ...
            if not self.use_loss_scaling:
              ops.append(
                  tf.cond(grad_ok, lambda: opt.apply_gradients(grads),
                          tf.no_op))
            else:
          ...
          if dev == devices[-1]:
            with tf.name_scope('Statistics'):
              ops.append(
                  autosummary(self.id + '/learning_rate', self.learning_rate))
              ops.append(
  ...
  def get_loss_scaling_var(self, device):
    ...
    if device not in self._dev_ls_var:
      with absolute_name_scope(
          self.scope + '/LossScalingVars'), tf.control_dependencies(None):
...
class Network:
  ...
  def _init_graph(self):
    ...
    self.output_templates = [out_expr
    ...
    self.vars = OrderedDict([(self.get_var_localname(var), var)
  ...
  def find_var(self, var_or_localname):
    ...
    return self.vars[var_or_localname] if isinstance(var_or_localname,
                                                     str) else var_or_localname
  ...
  def __getstate__(self):
    return {
        ...
        'variables': list(
            zip(self.vars.keys(), run(list(self.vars.values()))))
    }
  ...
  def copy_vars_from(self, src_net):
    ...
    name_to_value = run(
        {name: src_net.find_var(name) for name in self.vars.keys()})
    set_vars(
        {self.find_var(name): value for name, value in name_to_value.items()})
  ...
  def list_layers(self):
    patterns_to_ignore = [
        '/Setter', '/new_value', '/Shape', '/strided_slice', '/Cast', '/concat'
    ]
    ...
    def recurse(scope, parent_ops, level):
      ...
      # Otherwise => interpret as a layer.
        ...
        layer_trainables = [
            op.outputs[0] for op in ops if op.type.startswith('Variable') and
            self.get_var_localname(op.name) in self.trainables
        ]
  ...
  def print_layers(self, title=None, hide_layers_with_no_params=False):
    ...
    print()
    print('%-28s%-12s%-24s%-24s' % (title, 'Params', 'OutputShape',
                                    'WeightShape'))
    ...
    for layer_name, layer_output, layer_trainables in self.list_layers():
      ...
      print('%-28s%-12s%-24s%-24s' %
            (layer_name, num_params if num_params else '-', layer_output.shape,
